...
echo "Updating Pi Face Tracker and AS5600 test dependencies..."
sudo apt-get update
sudo apt-get install -y python3-pip python3-venv python3-opencv v4l-utils i2c-tools libopenblas-dev libjpeg-dev

mkdir -p "$PROJECT_DIR"
if [ "$SOURCE_DIR" != "$PROJECT_DIR" ]; then
    cp "$SOURCE_DIR/README.md" "$SOURCE_DIR/requirements.txt" "$SOURCE_DIR/face_tracker.py" "$SOURCE_DIR/as5600_test.py" "$SOURCE_DIR/installer.sh" "$SOURCE_DIR/update.sh" "$SOURCE_DIR/verify install.bash" "$PROJECT_DIR/"
fi
